...
namespace Productjxc.DAL
{
	...
	public class Contractor
	{
        ...
        #region  Method(厂商信息)
        /// <summary>
        /// 根据厂商号和厂商名获得厂商信息的数据列表
        /// </summary>
        public DataSet GetListContractor(string ConNO, string ConName)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ConNO,ConName,ConAddress ");
            strSql.Append(" FROM Contractor where 1=1");

            if (ConNO.Trim() != "")
            {
                strSql.Append(" and ConNO like '%" + ConNO + "%'");
            }
            if (ConName.Trim() != "")
            {
                strSql.Append(" and ConName like '%" + ConName + "%'");
            }

            return DbHelperSQL.Query(strSql.ToString());
        }
        #endregion  Method(厂商信息)
    }
}
